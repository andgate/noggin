...
import { z } from "zod";
import { gradedResponseSchema, GradedResponse, GradedSubmission } from "../types/quiz-generation-types";
import { Question, Quiz } from "../types/quiz-view-types";

const gradeQuizQuestionPrompt = (sources: string[], question: Question, response: string): string => `\
    \n\n${sources.join("\n\n---\n\n")}
...
export const gradeQuiz = async ({ quiz, responses }: GradeQuizOptions): Promise<GradedSubmission> => {
...
export const gradeResponse = async (
    ...
    const completion = await client.beta.chat.completions.parse({
        ...
        response_format: zodResponseFormat(z.object({ newQuestion: gradedResponseSchema }), "gradedResponse"),
    });
