...
test.describe("Quiz", () => {
    ...
    test("can create a new quiz", async ({ page }) => {
        test.setTimeout(5 * 60 * 10000); // Let it run for up to 5 minutes.
        ...
        // Fill in the form
        await page.getByLabel("Study Content").fill(WashingtonWikiPaste);
        ...
        // Verify the quiz view page is displayed with content
        await expect(page.getByTestId("quiz-view-page")).toBeVisible();
        await expect(page.getByTestId("quiz-questions-list")).toBeVisible();

        // Verify we have the expected number of questions
        const questionCount = await page.getByText(/Question \d+:/).count();
        expect(questionCount).toBe(3);

        // Verify at least one question has content
