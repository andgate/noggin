...
import { responses, submissions } from "drizzle/schema";
import { GradedSubmission } from "~/types/quiz-generation-types";
import { Quiz, Submission, SubmissionId } from "~/types/quiz-view-types";

export const submitQuiz = createServerFn<
    "POST",
    { quiz: Quiz; gradedSubmission: GradedSubmission },
    SubmissionId
>("POST", async ({ quiz, gradedSubmission }) => {
...
export const getSubmission = createServerFn<"GET", SubmissionId, Submission>(
    "GET",
    async (submissionId) => {
                ...
                question:
                    response.question.questionType === "multiple_choice"
                        ? {
                              ...
                              choices: response.question.choices.map(
                                  (choice) => ({
                                      id: choice.id,
